...
#if defined(NDEBUG)
# error "Eltoken cannot be compiled without assertions."
#endif
...
unsigned int nTargetSpacing = 64;

int nBlocksToMaturity = 200;
int nBestHeight = -1;
...
const string strMessageMagic = "Eltoken Signed Message:\n";
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (nBlocksToMaturity + 1) - GetDepthInMainChain());
}
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Eltoken miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            {
                int nSpendDepth;
                if (IsConfirmedInNPrevBlocks(txindex, pindexBlock, nBlocksToMaturity, nSpendDepth))
            ...
            if(IsLockedTxOut(txPrev.GetHash(), txPrev.vout[prevout.n]))
...
bool CTransaction::GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev, uint64_t& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        int nSpendDepth;
        if (IsConfirmedInNPrevBlocks(txindex, pindexPrev, nBlocksToMaturity - 1, nSpendDepth))
        {
...
{
#ifdef ENABLE_WALLET
    if(pwalletMain && pwalletMain->mapWallet.count(hash))
        return pwalletMain->mapWallet[hash].nHeight;
#endif
    CTransaction tx;
    uint256 hashBlock = 0;
...
bool IsLockedTxOut(const uint256& hash, const CTxOut& txout)
{
    ...
    int64_t nOffset = txout.nUnlockHeight - GetHeightFromTxHash(hash);
...
int GetLockMonth(const uint256& hash, const CTxOut& txout)
{
    if(txout.nUnlockHeight <= 0)
        return 0;

    int nHeight = GetHeightFromTxHash(hash);
    if(txout.nUnlockHeight < nHeight)
        return 0;

    int x1 = (txout.nUnlockHeight - nHeight) / (30 * 24 * 60);
    int x2 = (txout.nUnlockHeight - nHeight) % (30 * 24 * 60);
    if(x2 != 0)
        x1++;

    if(!LockMonthRange(x1))
        throw std::runtime_error("GetLockMonth() : locked month out of range");

    return x1;
}

int64_t GetLockPrincipal(const uint256& hash, const CTxOut& txout)
{
    int64_t nLockPrincipal = 0;
    int nLockMonth = GetLockMonth(hash, txout);
    double dValue = txout.nValue * 240 / (nLockMonth + 240.0);
    int64_t nValue = txout.nValue * 240 / (nLockMonth + 240);
    if(dValue > nValue)
        nLockPrincipal = nValue + 1;
    else
        nLockPrincipal = nValue;
    return nLockPrincipal;
}

int64_t GetLockInterest(const uint256& hash, const CTxOut& txout)
{
    return txout.nValue - GetLockPrincipal(hash, txout);
}
