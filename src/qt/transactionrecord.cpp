    ...
    else
    {
        ...
        if(nNet > 0) // receive from other
        {
            for(unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                ...
                if(wallet->IsMine(txout))
                {
                    ...
                    {
                        sub.lock = true;
                        sub.nUnlockHeight = txout.nUnlockHeight;
                        sub.nLockPrincipal = GetLockPrincipal(hash, txout);
                        sub.nLockInterest = txout.nValue - sub.nLockPrincipal;
                    }
        ...
        else
        {
            if (fAllFromMe && fAllToMe) // pay to self
            {
                ...
                if(!fContainLock) // none-lockced tx
                {
                ...
                else // locked tx
                {
                    ...
                    {
                        const CTxOut& txout = wtx.vout[nOut];
                        if(txout.nUnlockHeight == 0) // ignore change txout
                            continue;
            ...
            else if (fAllFromMe)
            {
                ...
                if(!fContainLock) // none-lockced tx
                {
                    ...
                    for(unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
                    {
                        ...
                        CTxDestination address;
                        if (ExtractDestination(txout.scriptPubKey, address)) // sent to Eltoken address
                        {
                ...
                else // locked tx
                {
                    ...
                    for(unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
                    {
                        ...
                        if(wallet->IsMine(txout)) // maybe send to self with locked txout
                        {
                            sub.type = TransactionRecord::SendToSelf;
                            sub.credit = txout.nValue;
                        }
