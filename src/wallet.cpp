...
bool CWallet::AddToWallet(const CWalletTx& wtxIn)
{
    uint256 hash = wtxIn.GetHash();
    {
        ...
        if(wtxIn.hashBlock != 0 && mapBlockIndex.count(wtxIn.hashBlock))
            wtx.nHeight = mapBlockIndex[wtxIn.hashBlock]->nHeight;
        else
            wtx.nHeight = nBestHeight + 1;
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsTrusted())
            {
                BOOST_FOREACH(const CTxOut& txout, pcoin->vout)
                {
                    if(!IsMine(txout))
                        continue;
                    if(IsLockedTxOut(pcoin->GetHash(), txout))
                        nTotal += txout.nValue;
                }
            }
        }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
                if (!(pcoin->IsSpent(i)) && IsMine(pcoin->vout[i]) && pcoin->vout[i].nValue >= nMinimumInputValue && !IsLockedTxOut(pcoin->GetHash(), pcoin->vout[i]) &&
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-eltoken-address
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                {
                    const CLockData& data = s.second;
                    wtxNew.vout.push_back(CTxOut(data.nAmount + GetUnlockReward(data.nAmount, data.nMonth), s.first, nBestHeight + 1 + data.nMonth * 30 * 24 * 60));
                }
...
uint64_t CWallet::GetStakeWeight() const
{
    ...
    {
        if (pcoin.first->GetDepthInMainChain() >= nBlocksToMaturity)
...
{
    CBigNum nReward = CBigNum(nAmount) * nMonth * 5 / 1200;
    return nReward.getuint64();
}
...
string CWallet::SendMoneyWithLock(CScript scriptPubKey, int64_t nLockAmount, int nLockMonth, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if(nLockAmount + nTransactionFee > GetBalance())
    ...
    {
        string strError;
        if (nLockAmount + nFeeRequired > GetBalance())
