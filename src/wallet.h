...
public:
    ...
    int64_t GetCredit(const uint256& hash, const CTxOut& txout, bool bContainLock = false) const
    {
        if (!MoneyRange(txout.nValue))
            throw std::runtime_error("CWallet::GetCredit() : value out of range");
        if(bContainLock)
            return (IsMine(txout) ? txout.nValue : 0);
        return (IsMine(txout) && !IsLockedTxOut(hash, txout) ? txout.nValue : 0);
    }
    ...
    int64_t GetCredit(const CTransaction& tx, bool bContainLock = false) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            nCredit += GetCredit(tx.GetHash(), txout, bContainLock);
...
public:
    ...
    (
        ...
        if (!fRead)
        {
            ...
            if(nHeight)
                pthis->mapValue["height"] = strprintf("%u", nHeight);
        }
        ...
        if (fRead)
        {
            ...
            pthis->nHeight = mapValue.count("height") ? atoi(pthis->mapValue["height"]) : 0;
        }
    ...
    int64_t GetAvailableCredit(bool bContainLock = false, bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                nCredit += pwallet->GetCredit(GetHash(), txout, bContainLock);
                if (!MoneyRange(nCredit))
